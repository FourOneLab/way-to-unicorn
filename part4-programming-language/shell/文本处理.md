...
find . -atime 7 -type f -print  # 最近7天被访问过的所有文件

# 按大小搜索：w 字 k M G
find . -type f -size +2k  # 寻找大于 2k 的文件
...
# 找到文件后的后续动作
find . -type f -name "*.swp" -delete  # 删除：删除当前目录下所有的 swp 文件

# 执行动作（强大的 exec）
find . -type f -user root -exec chown weber {} \; # 将当前目录下的所有权变更为weber
# 注：{} 是一个特殊的字符串，对于每一个匹配的文件，{} 会被替换成相应的文件名；

find . -type f -mtime +10 -name "*.txt" -exec cp {} OLD \;  # 将找到的文件全都 copy 到另一个目录：
...
```bash
grep match_patten file # 默认输出匹配行
...
grep "class" . -R -n  # 在多级目录中对文本递归搜索 (程序员搜代码）
...
cat single.txt | xargs -n 3  # 将单行转化为多行输出
...
sort unsort.txt | uniq -d  # 可指定每行中需要比较的重复内容：-s 开始位置 -w 比较字符数
```
...
paste file1 file2 -d “,”    # 默认的定界符是制表符，可以用 -d 指明定界符
...
echo -e “line1 f2 f3\n line2 \n line 3” | awk ‘{print NR”:”1”-“$2}’ # 打印每一行的第二和第三个字段：

awk '{print $2, $3}' file  # 统计文件的行数

awk ' END {print NR}' file  # 累加每一行的第一个字段

echo -e "1\n 2\n 3\n 4\n" | awk 'BEGIN{num = 0 ; print "begin";} {sum += $1;} END {print "=="; print sum }'  # 传递外部变量

var=1000echo | awk '{print vara}' vara=$var # 输入来自stdin
awk '{print vara}' vara=$var file # 输入来自文件 用样式对awk处理的行进行过滤
awk ‘NR < 5’ #行号小于 5
awk ‘NR==1,NR==4 {print}’ file #行号等于 1 和 4 的打印出来
awk ‘/linux/‘ #包含 linux 文本的行（可以用正则表达式来指定，超级强大）
awk ‘!/linux/‘ #不包含 linux 文本的行

awk -F  # ‘{print $NF}’ /etc/passwd  # 使用 - F 来设置定界符（默认为空格）
echo | awk '{"grep root /etc/passwd" | getline cmdout; print cmdout }'  # 使用 getline，将外部 shell 命令的输出读入到变量 cmdout 中；

for(i=0;i<10;i++){print $i;}for(i in array){print array[i];}    # 在 awk 中使用循环



seq 9| \ awk '{lifo[NR] = $0; lno=NR} \ END{ for(;lno>-1;lno--){print lifo[lno];} } '   # 以逆序的形式打印行：(tac 命令的实现）

awk 'NR<=10{print}' filename    # 类似head命令
awk '{buffer[NR%10] = $0;} END{for(i=0;i<11;i++){ \ print buffer[i %10]} } ' filename   # 类似tail命令

ls -lrt | awk '{print $6}'  # 打印指定行
ls -lrt | cut -f6   # 打印指定文本区域

seq 100| awk 'NR==4,NR==6{print}'   # 确定文本打印处于 start_pattern 和 end_pattern 之间的文本；
...
# 迭代每一个字符
${string:start_pos:num_of_chars}    # 从字符串中提取一个字符；(bash 文本切片）${#word}  # 返回变量 word 的长度
